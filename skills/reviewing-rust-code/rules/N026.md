# N026: integer-overflow-in-release

**Level:** default

## Problem

Integer arithmetic on user-influenced values silently wraps in release mode (Rust's default).
Debug mode panics on overflow, but release builds wrap, producing incorrect results without any indication of error.
Arithmetic silently produces a wrapped value — potentially allocating a huge buffer from a small multiplication, indexing the wrong element, or computing an incorrect total.

## Pros / Cons / Decision

**Pros of checked arithmetic:**
- Overflow produces an explicit error instead of a silently wrong value.
- Catches bugs that only manifest with production-sized inputs.

**Cons:**
- Checked methods return `Option`, adding error handling overhead.
- Overflow is impossible for many compile-time-bounded values.

**Decision:**
Use `checked_*` or `saturating_*` methods for arithmetic on values derived from external input.
