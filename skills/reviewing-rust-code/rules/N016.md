...
- Action function (imperative name like `validate`, `send`, `store`) that returns `bool` instead of `Result` — action functions should signal failure via `Result`, not via a return value the caller can silently ignore. Conversely, a predicate (`is_`, `has_`, `can_`) that returns `Result` instead of `bool` confuses callers who expect a simple yes/no answer. Inspired by [Linux kernel coding style §16](https://www.kernel.org/doc/html/v6.4/process/coding-style.html#function-return-values-and-names): *"If the name of a function is an action or an imperative command, the function should return an error-code integer. If the name is a predicate, the function should return a 'succeeded' boolean."*
