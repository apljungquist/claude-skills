# P019: quadratic-collection-ops

**Level:** default

## Problem

Nested iteration or repeated linear searches on Vec/slice create O(n^2) or worse behavior when a HashSet/HashMap would give O(n).
Code that works fine in development with small datasets becomes a bottleneck or causes timeouts in production with larger inputs.

## Pros / Cons / Decision

**Pros of using HashSet/HashMap for lookups:**
- O(1) lookups instead of O(n) per iteration.
- Prevents production performance cliffs with large datasets.

**Cons:**
- HashSet/HashMap have overhead for very small collections.

**Decision:**
Flag nested iteration with linear search on collections that could be large.
Small, bounded collections are exempt.
