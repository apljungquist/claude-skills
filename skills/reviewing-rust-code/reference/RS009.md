# RS009: broad-from-for-newtype

## Problem

`From<T>` is implemented for a semantically specific newtype where `T` is a general-purpose type (e.g., `From<u128> for UserId`).
This allows any value of the general type to silently convert into the domain type via `.into()`,
bypassing validation and making type confusion invisible at call sites.
A caller can pass a completely unrelated value — a directory ID, a timestamp, a byte count — and `.into()` will happily convert it to a `UserId` with no compiler warning.

## Pros / Cons / Decision

**Pros of explicit named constructors over From:**
- Call sites are explicit and reviewable: `UserId::from_raw(x)`.
- Prevents silent cross-type confusion via `.into()`.

**Cons:**
- Named constructors are more verbose than `.into()`.
- Measurement types where any value is valid are fine with `From`.

**Decision:**
Use named constructors for domain newtypes where not all source values are semantically valid.
...
- The Rust API Guidelines C-CONV-TRAITS [^1] recommend implementing `From` "where it makes sense" — this rule articulates one common case where it does not.

[^1]: https://github.com/rust-lang/api-guidelines/blob/97a0969cb07fe4cabb0eed8a56234053f47d83dc/src/interoperability.md#conversions-use-the-standard-traits-from-asref-asmut-c-conv-traits
